...
class TestBookingAuth:
    ...
    def setup(self, base_url: str):
        self.booking_auth: BookingAuth = BookingAuth(base_url)
        yield

    def test_token_should_be_created_success(self, setup, credentials: dict) -> None:
        response: Response = self.booking_auth.auth(json=credentials, headers={'Content-Type': 'application/json'})
    ...
    def test_token_request_without_credentials_should_fail(self, setup) -> None:
        response: Response = self.booking_auth.auth(headers={'Content-Type': 'application/json'})
